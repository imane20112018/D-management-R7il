...
use Illuminate\Support\Facades\Mail; // ✅ AJOUTE CECI
use App\Mail\RelanceAbonnementMail; // ✅si tu utilises un Mailable
...
class AbonnementController extends Controller
{
    ...
    public function indexExpires(Request $request)
    {
        $perPage = $request->get('per_page', 10);

        $abonnements = Abonnement::with('transporteur')
            ->where('statut', 'expire')
    ...
    /**
     * Relancer un transporteur dont l’abonnement est expiré
     */
    public function relancer($id)
    {
        $abonnement = Abonnement::with('transporteur')->findOrFail($id);

        if (!$abonnement->transporteur) {
            return response()->json(['message' => 'Transporteur introuvable'], 404);
        }

        $transporteur = $abonnement->transporteur;

        // 📩 Envoi email
        Mail::raw(
            "Bonjour {$transporteur->nom},\n\nVotre abonnement a expiré le {$abonnement->date_fin}. " .
                "Nous vous invitons à renouveler votre abonnement pour continuer à utiliser nos services.\n\nMerci.",
            function ($message) use ($transporteur) {
                $message->to($transporteur->email)
                    ->subject("Votre abonnement est expiré - Renouvellement");
            }
        );
